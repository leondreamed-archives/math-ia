...
The median wpm ratio of a word depends on various features of the word, such as the length of the word, the amount of capital letters of the word, etc.
...
\begin{align}
	\text{Difficulty} = w_{\text{word length}} * (\text{word length}) + Bias
\end{align}

However, when we plot the graph of word length against median WPM, we get the following figure:
...
One popular cost function used in machine learning is the MSE (Mean Squared Error). This function defines the cost based on the squares of the difference between the actual data ($y_i$) and the predicted data ($f(x_i)$):

\begin{align*}
	C(x) & = \frac{1}{N}\big[(y_1 - f(x_1))^2 + (y_2 - f(x_2))^2 + ... + (y_n - f(x_n))^2]
	\\
	C(x) & = \frac{1}{N} \sum_{i=1}^{n} (y_i - f(x_i))^2
\end{align*}
...
The goal of our machine learning algorithm is to minimize this function: to find a set of weights such that $C(x)$ is as low as possible. A popular method for minimizing cost is by using gradient descent.

\subsection*{Gradient Descent}

Gradient descent involves using the derivative of a function to find a function's local minimum.
