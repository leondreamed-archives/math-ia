...
\begin{align*}
	\text{WPM Ratio} = w_{\text{word length}} * (\text{word length}) + \text{Bias}
\end{align*}
...
\begin{align*}
	\text{WPM Ratio} = w_1 * (\text{Feature 1}) + w_2 * (\text{Feature 2}) + ... + \text{Bias}
\end{align*}
...
\begin{align*}
	\text{WPM Ratio} = w_{\text{word length}} * (\text{word length}) + w_{\text{capital letters}} * (\text{\# of capital letters}) + \text{Bias}
\end{align*}
...
Let us define the WPM Ratio as a function that takes in a word $W_i$ as input and returns the sum of the feature weights multiplied with the feature values of $W_i$. If $w_i$ represents the weight for feature $i$, $x_{f_i}$ represents the feature value for feature $f$ of word $i$, and $b$ represents the bias term, then our WPM ratio equation can be written as:
\begin{align*}
...
One popular cost function used in machine learning is the MSE (Mean Squared Error). This function defines the cost as the squares of the difference between the actual data ($y_i$) and the predicted data ($f(W_i)$):
\begin{align*}
	C(f) & = \frac{1}{N}\big[(y_1 - f(W_1))^2 + (y_2 - f(W_2))^2 + ... + (y_n - f(W_n))^2]
	\\
	C(f) & = \frac{1}{N} \sum_{i=1}^{n} (y_i - f(W_i))^2
\end{align*}
...
def get_expanded_summation():
	...
	for i, x, y in zip(range(len(x_values)), x_values, y_values):
		if i != 0 and i % 2 == 0:
			equation += "\\\\ &"
		equation += f"\\big[\\big({'%.3f' % y} - ({weight})({x}) - ({bias})\\big) * ({x})\\big]"
		if i != len(x_values) - 1:
			equation += "+"
	return equation
...
If we substitute these values for $w_1$ and $b$ into our prediction function $f(W_i)$ and plot our function on a graph alongside the data from Table 2, we obtain the following graph:
\begin{figure}[H]
	\centering
	\caption{A graph of our prediction function $f(W_i)$ using the values from equation 1}
