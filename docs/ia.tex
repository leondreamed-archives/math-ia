...
Because the value of $\frac{dC}{dw_1}$ is positive, it indicates that increasing $w_1$ and keeping $b$ constant would increase the value of our cost function $C(w_1, b)$. However, in machine learning, because our goal is to minimize this cost, we should decrease $w_1$.
...
This process is repeated for each partial derivative. For example, with our current bias value \py{bias}, the partial derivative $\frac{dC}{db}$ is equal to:
\begin{align*}
...
In this case, since $\frac{dC}{db}$ is negative when $b = \py{bias}$, it indicates that increasing $b$ will decrease the cost.
