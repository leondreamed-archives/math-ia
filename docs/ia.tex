...
Let us denote $\vec{X_i}$ as a vector of length $(\text{\# features}) + 1$ containing all the values of the features of $\text{Word}_i$. Let us denote $\vec{W}$ as a vector containing all the weights, including the value of the bias at the end. Using these vector definitions, we can simplify our prediction function:
\begin{align*}
	f(\text{Word}_i) & = \Big(\sum_{f=1}^{10} w_f{x_f}_i\Big) + b
	\\
	f(\vec{X_i})     & = \vec{X_i} \cdot \vec{W} + b
\end{align*}

To further simplify this equation, we can define $\vec{X}_i$ as a vector of length $(\text{\# features}) + 1$ instead, where in addition to containing all the values of the features of $\text{Word}_i$, it also includes a constant 1 at the end:
\begin{align*}
	\vec{X}_i = ({x_1}_i, {x_2}_i, \dots, {x_{10}}_i, 1)
\end{align*}

This way, if we redefined $\vec{W}$ to contain all the weights \textit{and} the bias term:
\begin{align*}
...
Not only do vectors allow us to represent our equations more succinctly, but they also make implementing them with code much easier.
