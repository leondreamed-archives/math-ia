...
Since the gradient at $w_1 = \py{weight}$ is positive, it indicates that the the slope at the point ($w_1$) is positive. To decrease the loss, we need to increase $w_1$ so that we are moving in the direction opposite to the direction where the gradient is increasing.

However, how do we know how much to move by? If we move too far, we could end up in a position of higher loss than we were before. If we move left too slowly, it will take a long time before we find the minimum.
